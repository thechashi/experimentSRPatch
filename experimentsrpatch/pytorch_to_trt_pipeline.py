...
@click.option(
    "--use_precision",
    default="fp32",
    help="Enables FP16 or FP32 precision for layers that support it, in addition to FP32",
)
...
def build_onnx_trt(model_name, patch_dim, use_precision, verbose):
    ...
    onnx_model_name = (
        model_name.lower() + "_" + str(use_precision) + "_" + str(patch_dim) + ".onnx"
    )
    # =============================================================================
    ...
    while True:
        ...
        if process_output.returncode != 0:
            ...
            patch_dim -= 1
            print(
                "Memory out. Decreasing patch size. New patch_size = {}".format(
                    patch_dim
                )
            )
            onnx_model_name = (
