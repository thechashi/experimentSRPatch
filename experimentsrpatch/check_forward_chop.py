"""
Source code for experimenting of iterative forward chop in a dimension range
"""
import sys
import subprocess
...
def check_differnet_patches_in_forward_chop(
    min_dim, max_dim, shave, image_path, gap=1, run=1, device="cuda"
):
    """
    ...
    for d in tqdm(range(min_dim, max_dim + 1, gap)):
        s = [0, 0, 0, 0, 0]
        ut.get_gpu_details(
            "cuda", "GPU status before patch: ({}x{}):".format(d, d), logger
        )
        for r in range(run):
            temp = [d]
            command = (
    ...
    plt.savefig(
        "results/forward_chop_experiment/{0}.png".format("processing_time_" + date)
    )
    plt.show()
    ...
    plt.plot(x_data, y_data)
    plt.savefig(
        "results/forward_chop_experiment/{0}.png".format("cropping_time_" + date)
    )
    plt.show()
    ...
    plt.plot(x_data, y_data)
    plt.savefig(
        "results/forward_chop_experiment/{0}.png".format("shfiting_time_" + date)
    )
    plt.show()
    ...
    plt.plot(x_data, y_data)
    plt.savefig(
        "results/forward_chop_experiment/{0}.png".format("cuda_cleaning_time_" + date)
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    check_differnet_patches_in_forward_chop(
        start_dim, end_dim, shave, image_path, run, device
    )
