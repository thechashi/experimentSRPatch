...
async def create_patch_queue(queue, img, dim, shave, img_height, img_width):
    patch_count = 0
    for patch_height_start in range(0, img_height, dim-2*shave):
        print('Inside asyncio')
        for patch_width_start in range(0, img_width, dim-2*shave):
            ...
            #queue.append(patch)
            queue.append(img[:, :, patch_height_start:patch_height_end, patch_width_start:patch_width_end])
...
def create_patch_queue_noasync(queue, img, dim, shave, img_height, img_width):
    patch_count = 0
    for patch_height_start in range(0, img_height, dim-2*shave):
        print('Inside syncio')
        for patch_width_start in range(0, img_width, dim-2*shave):
...
async def main(async_lr_queue, input_image, dim, shave, h, w):
    await asyncio.gather(create_patch_queue(async_lr_queue, input_image, dim, shave, h, w), loop())
...
if __name__ == "__main__":
    img_path = sys.argv[1] if len(sys.argv) > 1 else 'data/t2.jpg'
...
#         async_lr_queue = []
#         t0 = time.time()
#         asyncio.run(create_patch_queue(async_lr_queue, input_image, 64, 10, h, w))
#         #create_patch_queue_noasync(lr_queue, input_image, 64, 10, h, w)
#         
#         for i in range(1000):
#             for j in range(100):
#                 pass
#         #create_patch_queue(lr_queue, input_image, 64, 10, h, w)
#         elapsed = time.time() - t0
#         async_times.append(elapsed)
#     print('Asyns Took %.6f ms on avg' % (1000*(statistics.mean(async_times))))
#     print('Async Standard Deviation: %.6f ms ' % (1000*(statistics.stdev(async_times))))
#     
#     run = 500
#     sync_times = []
#     for r in range(run):
#         sync_lr_queue = []
#         t0 = time.time()
#         #asyncio.run(create_patch_queue(lr_queue, input_image, 64, 10, h, w))
#         create_patch_queue_noasync(sync_lr_queue, input_image, 64, 10, h, w)
#         
#         for i in range(1000):
#             for j in range(100):
#                 pass
...
#     print('Sync Took %.6f ms on avg' % (1000*(statistics.mean(sync_times))))
#     print('Sync Standard Deviation: %.6f ms ' % (1000*(statistics.stdev(sync_times))))
# =============================================================================
    async_lr_queue = []
    asyncio.run(main(async_lr_queue, input_image, 40, 10, h, w))
    print()
# =============================================================================
#     asyncio.run(create_patch_queue(async_lr_queue, input_image, 32, 10, h, w))
#     #create_patch_queue_noasync(async_lr_queue, input_image, 64, 10, h, w)
#     
#     for i in range(100):
#         print('outer loop')
#         for j in range(100):
