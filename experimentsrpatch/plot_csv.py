...
def plot_two_lines(file1, file2, x1, y1, x2, y2):
    ...
    # Creating plot title
    plt_title = "Recursive vs Iterative forward chop (GeForce GTX 860M)"

    # X axis - Patch dimension, Y axis - Maximum batch size
    x1d, y1d = (
        np.round(np.array(fr1.iloc[::1, x1].values).flatten(), 2),
        np.round(np.array(fr1.iloc[::1, y1].values).flatten(), 2),
    )
    x2d, y2d = (
        np.round(np.array(fr2.iloc[::1, x2].values).flatten(), 2),
        np.round(np.array(fr2.iloc[::1, y2].values).flatten(), 2),
    )
    # =============================================================================
    #     x_data, y_data = (
    #         np.round(np.array(full_result.iloc[::4, x_column_index].values).flatten(), 2),
    ...
    plt.ylabel(y_label)
    plt.title(plt_title)
    plt.plot(x1d, y1d, linestyle="--", marker="o", color="r", label="Recursive")
    plt.plot(x2d, y2d, linestyle="--", marker="o", color="b", label="Iterative")
    # plt.plot(x_data, y_data, linestyle="--", marker="o", color="r")
    # ax.plot(x, y, 'bo-')
...
if __name__ == "__main__":
    # main()
    plot_two_lines(
        "results/recursive_resultTue_Aug_10_14_45_46_2021.csv",
        "results/patch_iterative_resultTue_Aug_10_14_05_11_2021.csv",
        3,
        1,
        6,
        1,
